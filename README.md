...
A fully containerized monitoring and alerting stack built with Prometheus, Grafana, Node Exporter, Alertmanager, Docker Compose, and GitHub Actions. Collects system-level metrics, visualizes infrastructure performance through a custom Grafana dashboard, evaluates alerting rules, and sends Discord notifications when predefined conditions are triggered — fully reproducible with a single Docker Compose deployment.
...
1. Node Exporter exposes system-level metrics.
2. Prometheus scrapes and stores those metrics.
3. Grafana queries Prometheus and visualizes current and historical values.
4. Prometheus alert rules continuously evaluate infrastructure conditions.
5. When a rule enters a firing state, Prometheus forwards the alert to Alertmanager.
6. Alertmanager routes the notification to Discord.
...
| PromQL | Metric queries and alert expressions |
...
| YAML / JSON | Configuration and dashboard provisioning |
...
| HighMemoryUsage | Memory usage above 90% for 2 minutes | Critical |
| LowDiskSpace | Available disk space below 15% for 5 minutes | Warning |
| TargetDown | Prometheus target unreachable for 1 minute | Critical |
...
![Prometheus Alert](screenshots/prometheus-alert.png)
...
The real Discord webhook URL is not committed to the repository. It is stored locally in `discord_webhook_url` and excluded through `.gitignore`.
...
Grafana is automatically configured when the stack starts. The repository contains:
...
The project includes a GitHub Actions workflow at `.github/workflows/ci.yml`, which runs automatically on pushes and pull requests targeting `main`.
...
Create a file named `discord_webhook_url` in the project root and paste your Discord webhook URL into that file. Do not commit this file — it is excluded through `.gitignore`.
...
Prometheus detects the target as unavailable. After the configured delay, the `TargetDown` rule enters the FIRING state and Alertmanager forwards the notification to Discord. Restart Node Exporter with:
